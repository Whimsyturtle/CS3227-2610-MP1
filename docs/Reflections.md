...
5. **Be Wary of the Agentic Environment**: When running my [experiment](#3-comparing-basic-and-structured-test-prompts), starting each trial from the same commit did not provide sufficient isolation as Git metadata, build caches, etc. could affect the **context**, and thus contaminate the experiment's results. In the future, when I run **agentic LLM** experiments, I plan on controlling the full environment visible to the agent, beyond just the source files.
