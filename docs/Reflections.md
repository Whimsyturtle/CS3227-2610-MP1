...
2. **Project Intent**: I found that providing **context** (via repository access) did not guarantee understanding of project intent (e.g. it [missed parser conventions and preferred using non-standard terminology](../logs/011-implementing-zones-command.md), described ordinary TSV data using [invented terms such as "roster document", "complete roster", and "Quorum tab-separated format"](../logs/008-implementing-local-roster-persistence.md), and [repeatedly treated one meeting algorithm as the product's permanent identity](../logs/031-creating-and-refining-quorum-user-guide.md)).
...
5. **Update Code, Tests, and Documentation Together**: Although I developed this project iteratively, I largely updated the code, tests, and documentation in separate phases. In the future, when pushing features, I would update them together. I believe this would surface issues earlier and reduce inconsistencies, as the design context would still be fresh in my mind.
6. **Be Wary of the Agentic Environment**: When running my [experiment](#3-comparing-basic-and-structured-test-prompts), starting each trial from the same commit did not provide sufficient isolation as Git metadata, build caches, etc. could affect the **context**, and thus contaminate the experiment's results. In the future, when I run **agentic LLM** experiments, I plan on controlling the full environment visible to the agent, beyond just the source files.
